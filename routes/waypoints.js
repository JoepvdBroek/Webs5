...
function getWaypoints(req, res){
	...
	/*if(query.pagetoken !== undefined){
		...
		https.get("https://maps.googleapis.com/maps/api/geocode/json?address="+query.search,function(response){
			var data = '';
			...
			response.on('end',function(){
		        ...
		        if(data.results.length){
			        ...
			        url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location="+geoCord.lat+","+geoCord.lng+"&radius=1000&types=cafe&rankBy=distance&key="+configAuth.googleAuth.APIKey;
					RetrieveData(res,url);
		        }else{
		        	return handleError(req, res, 404, "No Data found"); 
		        }
			});
		});
	//}
